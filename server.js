  ...
  <section id="app" class="app hidden">
    ...
    <main class="main">
      ...
      <section class="content">
        ...
        <div id="page-products" class="page"><div class="page-head"><div><div class="page-title">Ürünler</div><div class="page-sub">Tüm ikas ürünleri.</div></div><div class="head-tools"><input class="input search-box" data-product-search placeholder="Ürün ara..."><button class="btn gold" data-route="ikas-products">Ürünleri Aç</button></div></div><div id="productsGrid" class="panel-body"><div class="empty">Ürünler yükleniyor...</div></div></div>

        <div id="page-ikas-orders" class="page"><div class="page-head"><div><div class="page-title">Tüm Siparişler</div><div class="page-sub">Bütün siparişler tarihe göre; en güncel sipariş en üstte.</div></div><div class="head-tools"><div class="date-filter" data-date-filter><select class="date-select" data-date-preset><option value="today">Bugün</option><option value="yesterday">Dün</option><option value="this_week">Bu hafta</option><option value="last_week">Geçen hafta</option><option value="this_month">Bu ay</option><option value="last_month">Geçen ay</option><option value="this_year">Bu yıl</option><option value="custom">Özel tarih</option><option value="all">Tüm zamanlar</option></select><div class="date-custom"><input class="date-input" type="date" data-date-start><span class="time">-</span><input class="date-input" type="date" data-date-end></div></div><input class="input search-box" data-order-search placeholder="Sipariş ara..."><button class="btn gold ikas-refresh">Senkronize Et</button></div></div><div id="ikasAllOrders" class="panel-body"><div class="empty">Siparişler yükleniyor...</div></div></div>
        <div id="page-ikas-products" class="page"><div class="page-head"><div><div class="page-title">Tüm Ürünler</div><div class="page-sub">Ürün fotoğrafı, varyant, SKU ve stok bilgileri.</div></div><div class="head-tools"><input class="input search-box" data-product-search placeholder="Ürün ara..."><button class="btn gold ikas-refresh">Senkronize Et</button></div></div><div id="ikasAllProducts" class="panel-body"><div class="empty">Ürünler yükleniyor...</div></div></div>
...
(function(){
  ...
  function pagerHtml(target,page,totalPages){if(totalPages<=1)return ''; var prev=Math.max(1,page-1),next=Math.min(totalPages,page+1); var html='<div class="pager"><button data-page-target="'+target+'" data-page="'+prev+'" '+(page<=1?'disabled':'')+'>‹</button>'; var start=Math.max(1,page-2),end=Math.min(totalPages,page+2); if(start>1)html+='<button data-page-target="'+target+'" data-page="1">1</button>'; if(start>2)html+='<span class="time">...</span>'; for(var i=start;i<=end;i++){html+='<button class="'+(i===page?'active':'')+'" data-page-target="'+target+'" data-page="'+i+'">'+i+'</button>'} if(end<totalPages-1)html+='<span class="time">...</span>'; if(end<totalPages)html+='<button data-page-target="'+target+'" data-page="'+totalPages+'">'+totalPages+'</button>'; html+='<button data-page-target="'+target+'" data-page="'+next+'" '+(page>=totalPages?'disabled':'')+'>›</button><span class="time">Sayfa '+page+' / '+totalPages+'</span></div>'; return html}
